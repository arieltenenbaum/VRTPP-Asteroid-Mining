...
This is a research project that is trying to find a better intializtaion method for the VRTPP-PR framework that Choi and Ho produce.
...
| Metric | Paper | `main` | `periapsis-init` |
|--------|-------|-------------------|------------------------------|
...
|--------|---------|--------|
| `main` | Core MILP–NLP optimizer with NLP warm-start + 2D grid initialization for `VRTPP_PR_Optimization.ipynb` |  **obj ≈ 18.93** (1 spacecraft, 3 mining visits: Earth→Anteros→Bennu→1989 ML→Earth, ~23 soft iters) |
...
- Used in `main` only; serves as controlled reference
...
- Use run_notebook.sh to see results
...
### What `VRTPP_PR_Optimization.ipynb` on `main` actually does 
...
3. ✅ **Already implemented** in both `main` and `periapsis-init`: `TimeLimit=100.0` is set in `solve_vrtpp_pr` (Cell 24) and `MIPGap=0.03` in `build_milp` (Cell 18). For `verification-suite`: sync `VRTPP_PR_Optimization.ipynb` from `periapsis-init` per the Notebook Sync Policy — this will bring both settings in automatically. For this branch there will be different time limits and MIP gaps based on scalability of n_r and n_m. 
4. Change or slightly modify research questions and experiments accordingly in `solution_design.md` in the `periapsis-init` branch to effectively show that the initialization strategy in the `VRTPP_PR_Optimization.ipynb` in the `peripasis-init` branch is more effective in several different ways than the initialization strategy in the `VRTPP-Papermodel.ipynb` in the `main` branch which is trying to replicate the one from the paper. 
5.  Commit all changes in GitHub and commit teh chnages in this .md file to all branches. 
...
| `MODEL_COMPARISON_AND_VALIDATION.md` | all | Paper vs. implementation comparison; notebook comparison (§9) | is definitely outdated and I would not use
